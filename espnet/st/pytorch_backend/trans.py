...
def trans_waitk(args):
    ...
    with torch.no_grad():
        for idx, name in enumerate(js.keys(), 1):
            ...
            #hyp_word_indices = word_splitter(nbest_hyps[0]['yseq'], 28, model.dec.eos)
            ...
            #ref_word_indices = word_splitter(torch.tensor(ref_seq), 28, model.dec.eos)
            #ref_word_indices = word_splitter_spm(torch.tensor(ref_seq), 819, 1986, model.dec.eos)
            #ref_word_indices = word_splitter_spm(torch.tensor(ref_seq), 263, 393, model.dec.eos)
