...
class SimultaneousICASSP21Decoder(torch.nn.Module, ScorerInterface):
    ...
    def forward(self, hs_pad, hlens, step, att_idx, z_list, c_list, att_w, z_all, eys=None):
        ...
        for i in six.moves.range(self.N):
            ...
            print(type(ey), type(z_list, type(c_list)), len(z_list), len(c_list), z_list[0].size(), c_list[0].size())
        ...
        for enc_step, (hs_pad, hlens) in enumerate(zip(hs_pad_list, hlens_list)):
            ...
            for i in six.moves.range(N):
                if self.num_encs == 1:
                    ...
                    self.att[att_idx].reset()
                    att_c, att_w = self.att[att_idx](hs_pad[0], hlens[0], self.dropout_dec[0](z_list[0]), att_w)
                    print("input + att size: ", hs_pad[0].size(), att_w.size())
        ...
        if len(z_all) < olength:
            ...
            for i in range(len_, olength):
                if self.num_encs == 1:
                    ...
                    self.att[att_idx].reset()
                    att_c, att_w = self.att[att_idx](hs_pad[0], hlens[0], self.dropout_dec[0](z_list[0]), att_w)
                    print("input + att size end: ", hs_pad[0].size(), att_w.size())
                    logging.info("input + att size end: " + str(hs_pad[0].size()) + ' ' + str(att_w.size()))
                else:
                    for idx in range(self.num_encs):
                        att_c_list[idx], att_w_list[idx] = self.att[idx](hs_pad[idx], hlens[idx],
        ...
        for i in six.moves.range(maxlen):
            ...
            for hyp in hyps:
                ...
                if lpz[0] is not None:
                    ...
                    local_best_scores, local_best_ids = torch.topk(local_scores, beam, dim=2)
