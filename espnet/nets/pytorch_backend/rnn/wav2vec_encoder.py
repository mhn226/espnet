...
class VGG2L(torch.nn.Module):
    ...
    def __init__(self, idim_reduction=False, in_channel=1):
        super(VGG2L, self).__init__()

        # Add a linear layer to reduce feature dimension
        self.idim_reduction = idim_reduction
        if self.idim_reduction:
            self.proj = torch.nn.Linear(512, 83)
    ...
    def forward(self, xs_pad, ilens, **kwargs):
        ...
        if self.idim_reduction:
            xs_pad = F.relu(self.proj(xs_pad))
...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, etype, idim, elayers, eunits, eprojs, subsample, dropout, idim_reduction, in_channel=1):
        ...
        if etype.startswith("vgg"):
            if etype[-1] == "p":
                self.enc = torch.nn.ModuleList([VGG2L(idim_reduction, in_channel),
...
def wav2vec_encoder_for(args, idim, subsample):
    # args.idim_reduction: reduce input dimension to 83
    if args.idim_reduction:
        idim=83
    return Encoder(args.etype, idim, args.elayers, args.eunits, args.eprojs, subsample, args.dropout_rate, args.idim_reduction)
