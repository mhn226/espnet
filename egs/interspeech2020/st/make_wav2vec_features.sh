...
if [ $# -lt 1 ] || [ $# -gt 3 ]; then
  cat >&2 <<EOF
Usage: $0 [options] <data-dir> [<log-dir> [<feats-dir>] ]
 e.g.: $0 data/train
Note: <log-dir> defaults to <data-dir>/log, and
      <feats-dir> defaults to <data-dir>/data
Options:
...
for n in $(seq $nj); do
  # the next command does nothing unless $featsdir/storage/ exists, see
  # utils/create_data_link.pl for more info.
  utils/create_data_link.pl $featsdir/wav2vec_$name.$n.ark
done
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:4 $logdir/make_wav2vec_${name}.JOB.log \
    python3 wav2vec_feature_extract.py $wav2vec_model_path $logdir/segments.JOB $logdir/tmp.JOB.ark $featsdir/raw_wav2vec_$name.JOB.ark \
         $featsdir/raw_wav2vec_$name.JOB.scp $write_utt2dur_opt $write_num_frames_opt $logdir JOB \
     || exit 1;

  $cmd JOB=5:8 $logdir/make_wav2vec_${name}.JOB.log \
    python3 wav2vec_feature_extract.py $wav2vec_model_path $logdir/segments.JOB $logdir/tmp.JOB.ark $featsdir/raw_wav2vec_$name.JOB.ark \
         $featsdir/raw_wav2vec_$name.JOB.scp $write_utt2dur_opt $write_num_frames_opt $logdir JOB \
     || exit 1;

  $cmd JOB=9:12 $logdir/make_wav2vec_${name}.JOB.log \
    python3 wav2vec_feature_extract.py $wav2vec_model_path $logdir/segments.JOB $logdir/tmp.JOB.ark $featsdir/raw_wav2vec_$name.JOB.ark \
         $featsdir/raw_wav2vec_$name.JOB.scp $write_utt2dur_opt $write_num_frames_opt $logdir JOB \
     || exit 1;

  $cmd JOB=13:16 $logdir/make_wav2vec_${name}.JOB.log \
    python3 wav2vec_feature_extract.py $wav2vec_model_path $logdir/segments.JOB $logdir/tmp.JOB.ark $featsdir/raw_wav2vec_$name.JOB.ark \
         $featsdir/raw_wav2vec_$name.JOB.scp $write_utt2dur_opt $write_num_frames_opt $logdir JOB \
     || exit 1;
  ...
  #   ark,scp:$fbankdir/wav2vec_$name.JOB.ark,$fbankdir/wav2vec_$name.JOB.scp \
  #   || exit 1;
fi
