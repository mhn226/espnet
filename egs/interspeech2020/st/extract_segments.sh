...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_wav2vec_${name}.JOB.log \
    extract-segments --min-segment-length=0 scp,p:$scp $logdir/segments.JOB ark:$logdir/tmp.JOB.ark \
     || exit 1;

fi
echo "$0: Succeeded extracting segments for $name"
