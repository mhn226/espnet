...
if [ ${set} = "train_nodevtest" ] || [ ${set} = "train_nodevtest_sp" ] || [ ${set} = "dev" ] || [ ${set} = "test" ]; then
    ...
    sort data/${set}/text_noseg.tc.pt > data/${set}.pt/text_noseg.tc
    sort data/${set}/text_noseg.lc.pt > data/${set}.pt/text_noseg.lc
    sort data/${set}/text_noseg.lc.rm.pt > data/${set}.pt/text_noseg.lc.rm
fi
utils/fix_data_dir.sh --utt_extra_files "text.tc text.lc text.lc.rm" data/${set}.pt
if [ ${set} = "train_nodevtest" ] || [ ${set} = "train_nodevtest_sp" ] || [ ${set} = "dev" ] || [ ${set} = "test" ]; then
    if [ -f data/${set}.pt/feats.scp ]; then
        utils/validate_data_dir.sh data/${set}.pt || exit 1;
    else
        utils/validate_data_dir.sh --no-feats data/${set}.pt || exit 1;
    fi
else
    if [ -f data/${set}.pt/feats.scp ]; then
        utils/validate_data_dir.sh --no-text data/${set}.pt || exit 1;
    else
        utils/validate_data_dir.sh --no-text --no-feats data/${set}.pt || exit 1;
    fi
fi
